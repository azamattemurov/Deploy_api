...
from post.models import PostModel, PostCommentModel, PostLikeModel, CommentLikeModel
from post.permissions import IsoOwner
...
class UserLikedPostsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        liked_posts = PostLikeModel.objects.filter(user=self.request.user)
        return PostModel.objects.filter(id__in=liked_posts)
...
class UserLikedCommentsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        liked_comments = CommentLikeModel.objects.filter(user=self.request.user)
        return PostCommentModel.objects.filter(id__in=liked_comments)
